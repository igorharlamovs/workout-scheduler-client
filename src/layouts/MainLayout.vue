  ...
  <q-layout view="lHh Lpr lFf" class="background">
    <q-footer bordered class="bg-grey-10 text-primary">
      <q-tabs no-caps active-color="primary" indicator-color="transparent" class="text-orange" v-model="tab">
        <q-route-tab v-if="userStore.user.id" name="home" label="Home" icon="house" to="/" exact />
        <q-route-tab v-if="userStore.user.id" name="plans" label="Plans" icon="event_note" to="/planlist" exact />
        <q-route-tab v-if="userStore.user.id" name="workouts" label="Workouts" icon="fitness_center" to="/workouts" exact />
